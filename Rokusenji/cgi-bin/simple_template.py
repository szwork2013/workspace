...
class SimpleTemplate(object):
    ...
    def __init__(self, body='', file_path=None):
        ...
        if file_path:
            f=open(file_path)
            body=f.read()#unicode(f.read(), 'utf-8', 'ignore')
        warning(type(body))
        body=unicode(body.decode("utf-8"))
        warning(type(body))
        body=body.replace('\r\n', '\n')
    ...
    def handle_value(self, _match, _line_no, _kws={}):
        ...
        while True:
            ...
            if not _m:
                break
            pos+=_m.end()
            _rep.append( (_m.group(1), eval(_m.group(1))) )#unicode(eval(_m.group(1)))) )
        for t, r in _rep:
            _line=_line.replace('${%s}'%t, str(r))
